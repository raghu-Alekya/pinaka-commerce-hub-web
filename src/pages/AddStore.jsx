...
import { listMerchantEmployees } from "../api/employees_stores";
...
export default function AddStore() {
  ...
  const merchant =
    merchantInfo?.owner ||
    merchantRecord ||
    merchants.find((item) => idOf(item) === merchantId) ||
    null;
  const merchantName =
    merchant?.name ||
  ...
  // Plan resolution from active subscription or merchant
  const plan =
  ...
  const storeLimit = Number(
    subscription?.maxStoresAllowed ??
      ...
      subscription?.licensed_store_count ??
      plan?.included_stores ??
      plan?.includedStores ??
      NaN,
  );

  const planFeatureSources = [
  ...
  const planFeatureList =
    planFeatureSources.find(
      (items) => Array.isArray(items) && items.length > 0,
    ) ?? planFeatureSources.find(Array.isArray);
  ...
  const filteredFeatures = useMemo(() => {
    return featureRows.filter((row) => {
      ...
      const matchesSearch =
        !featureSearch ||
        row.name.toLowerCase().includes(featureSearch.toLowerCase()) ||
        (row.description &&
          row.description.toLowerCase().includes(featureSearch.toLowerCase()));
      return matchesCategory && matchesSearch;
    });
  }, [featureRows, featureCategory, featureSearch]);

  const roleDefinitions = (
    Array.isArray(merchantInfo?.roles) ? merchantInfo.roles : []
  )
    .map((role) =>
      typeof role === "string"
        ? { name: role }
  ...
  const entitled = (name) => {
    ...
    if (activeStoreTypeId && typeof feature?.included === "boolean")
      return feature.included;
    if (activeStoreTypeId && feature?.planAccess)
      return String(feature.planAccess).toUpperCase() === "INCLUDED";
    if (includedFeatures.length)
      return includedFeatures.some(
        (item) => item.toLowerCase() === name.toLowerCase(),
      );
    if (planFeatureList != null) return false;
    if (typeof feature?.included === "boolean") return feature.included;
    if (feature?.planAccess)
      return String(feature.planAccess).toUpperCase() === "INCLUDED";
      ...
      try {
        ...
        if (storeId) {
          ...
          if (!cancelled) {
            ...
            const savedRoles = (saved.storeRoles || [])
              .map((role) =>
            ...
            const assignments =
              saved.employeeAssignments || saved.storeEmployees || [];
            setEmployeeAssignments(
              assignments
                .map((item) => ({
                  employeeId: String(
                    item.employeeId ?? item.id ?? item.employee?.id ?? "",
                  ),
                  role: item.role || item.storeRole || "",
                }))
  ...
  // Load Merchant Context & Employees
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadMerchantContext() {
      try {
        const [result, employeeResult] = await Promise.all([
          ...
          listMerchantEmployees(merchantId).catch((err) => {
            console.warn("Failed to load active merchant employees:", err);
            return [];
          }),
        ]);
        if (cancelled) return;
        ...
        setEmployees(
          employeeRows
            .filter(
              (employee) =>
                !employee.merchantId ||
                String(employee.merchantId) === merchantId ||
                !employee.merchant_id ||
                String(employee.merchant_id) === merchantId,
            )
            .map((employee) => ({
              ...employee,
              id: String(
              ...
              name:
                ...
                "Employee",
              phone:
                employee.phone || employee.phoneNumber || employee.mobile || "",
        ...
        if (!editing) {
          ...
          const currency =
            info.owner.currency || COUNTRIES[country]?.currency || "";

          setStore((current) => ({
            ...current,
            merchantId,
            type: info.typeName || current.type,
            storeTypeId: info.typeId || current.storeTypeId,
            country: country || current.country,
            currency: currency || current.currency,
            timezone: info.owner.timezone || current.timezone,
            defaultLanguage:
              info.owner.defaultLanguage || current.defaultLanguage,
          }));
          const inherited = (Array.isArray(info.roles) ? info.roles : [])
            .map((role) =>
              typeof role === "string" ? role : role.name || role.roleName,
            )
            .filter(Boolean);
          setRoles([...new Set(inherited)]);
          setActiveRole(inherited[0] || "");
        }
      } catch (err) {
        if (!cancelled)
          setMerchantError(
      ...
      setPermissions((current) => ({
        ...
        [role]: Object.fromEntries(
          featureRows.map((feature) => [
            feature.name,
            Object.fromEntries(
              feature.actions.map((action) => [action, false]),
            ),
          ]),
        ),
      }));
  };
  const togglePermission = (role, feature, action) =>
    setPermissions((current) => ({
      ...current,
      [role]: {
        ...current[role],
        [feature]: {
          ...
          [action]: !current[role]?.[feature]?.[action],
        },
      },
    }));
  ...
  const validateStep = (index, draftOnly = false, subOverride = null) => {
    ...
    if (index === 0) {
      ...
      if (!["Active", "Inactive", "Draft"].includes(store.status))
        return "Select a valid store status.";
  ...
  async function submit(event, saveDraft = false) {
    ...
    try {
      const storePayload = {
        ...
        storeRoles: roles,
        storeRolePermissions: Object.fromEntries(
          roles.map((role) => [
            role,
            Object.fromEntries(
              featureRows
                ...
                .map((feature) => [
                  ...
                  Object.fromEntries(
                    feature.actions.map((action) => [
                      action,
                      Boolean(permissions[role]?.[feature.name]?.[action]),
                    ]),
                  ),
                ]),
            ),
    ...
    return (
      <>
        <Panel
          ...
          subtitle="Choose roles available for this merchant."
        >
          {!roleDefinitions.length && (
            <p className="sf-empty">
              No role templates are available. Configure roles on the merchant
          ...
          <div className="sf-template-grid">
            {roleDefinitions.map((role, index) => (
              <label
                className={`sf-template ${roles.includes(role.name) ? "selected" : ""}`}
                key={role.name}
              >
                <input
                  type="checkbox"
                ...
                />
                <span className={`sf-template-icon icon-${index}`}>
                      ...
                      <td className="sf-pin-cell">
                        {editingPin ? (
                          ...
                          <div className="sf-pin-action">
                            ...
                            >
                              {assignment?.pin ? "Change PIN" : "Assign PIN"}
                            </button>
                          </div>
                        )}
                      </td>
                  ...
                  <tr>
                    <td colSpan="10" className="sf-empty">
                      {employees.length
                        ...
                        : "No active employees found for this merchant."}
                    </td>
                  </tr>
                )}
              </tbody>
