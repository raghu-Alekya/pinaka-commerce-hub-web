...
const idOf = (value) =>
  String(
    typeof value === "object"
      ? (value.vendorId ?? value.id ?? value._id ?? "")
      : (value ?? ""),
  );
...
const normalize = (value) => ({
  ...
  name: value.name || value.vendorName || "Unnamed vendor",

  type: value.type || value.vendorType || "—",
  ...
  assignedStoreCount: Array.isArray(value.stores)
    ? value.stores.length
    : Number(
...
const matches = (vendor, query) =>
  [vendor.name, vendor.type, vendor.contact, vendor.email, vendor.phone]
    .join(" ")
    .toLowerCase()
...
export default function MerchantVendors({
  ...
  const [masterVendors, setMasterVendors] = useState(initialMasterVendors);
  ...
  const [mappedError, setMappedError] = useState("");
  ...
  async function loadMappedVendors(searchValue = "") {
    ...
    try {
      const mapped = await getMappedMerchantVendors(merchantId, {
        search: searchValue,
      });
      ...
      setMasterVendors((current) => {
        const byId = new Map(current.map((vendor) => [idOf(vendor), vendor]));
      ...
      console.error("Failed to load mapped merchant vendors:", e);
      setMappedError(e?.message || "Unable to load mapped vendors.");
    } finally {
  ...
  const rows = assigned
    .filter((vendor) => matches(vendor, query))
    .sort(
      (a, b) =>
  ...
  const available = vendors.filter(
    (vendor) => !ids.includes(vendor.id) && matches(vendor, search),
  );
  ...
  async function openAddVendorModal() {
    ...
    try {
      const vendors = await getAvailableMerchantVendors(merchantId);

      setMasterVendors(Array.isArray(vendors) ? vendors : []);

      setModal({
        type: "select",
      });
    } catch (e) {
      console.error("Failed to load available merchant vendors:", e);

      setMasterError(e?.message || "Unable to load available vendors.");
  ...
  async function save(next) {
    ...
    try {
      const addedIds = next.filter((id) => !ids.includes(id));

      if (modal?.type === "remove") {
        await unmapMerchantVendor(merchantId, modal.vendor.id);
      } else if (modal?.type === "select") {
        if (!addedIds.length) {
          throw new Error("At least one vendor must be selected.");
        }

        await addMerchantVendors(merchantId, addedIds);
      }

      await loadMappedVendors();
      setModal(null);
    } catch (e) {
      console.error("Failed to save merchant vendor mapping:", e);

      setSaveError(e?.message || "Unable to save vendor assignment.");
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  const badge = (vendor) => (
    <span
      className={`mv-badge ${active(vendor) ? "mv-active" : "mv-inactive"}`}
    >
      ● {vendor.status}
    </span>
  );
    ...
    return (
      <tr>
        {select && (
          <th>
            <input
              ...
              checked={
                selectable.length > 0 &&
                selectable.every((vendor) => selection.includes(vendor.id))
              }
                ...
                setSelection((old) =>
                  e.target.checked
                    ? [
                        ...new Set([
                          ...old,
                          ...selectable.map((vendor) => vendor.id),
                        ]),
                      ]
                    : old.filter(
                        (id) => !selectable.some((vendor) => vendor.id === id),
                      ),
                )
              }
            />
        ...
        {(select
          ...
          : ["name", "type", "contact", "assignedStoreCount", "status"]
        ).map((key) => (
          <th key={key}>
            {select ? (
              {
                ...
                type: "Type",
                contact: "Contact",
                assignedStoreCount: "Assigned Stores",
                status: "Status",
              }[key]
            ) : (
              <button
    ...
    <div className="merchant-vendors">
      ...
      <header className="mv-card mv-header">
        <div>
          ...
          <p>These are the vendors connected to this merchant.</p>
        </div>
        ...
        >
          {masterLoading ? "Loading Vendors…" : "＋ Add Vendor"}
        </button>
      </header>
      ...
      <section className="mv-card">
        ...
        {loading || mappedLoading ? (
          <p role="status">Loading vendors…</p>
        ) : error || mappedError ? (
          <p role="alert">{error || mappedError}</p>
        ) : (
          <>
              ...
              <table>
                ...
                <tbody>
                  {rows
                    ...
                    .map((vendor) => (
                      <tr key={vendor.id}>
                        ...
                        <td>{vendor.assignedStoreCount}</td>
                  ...
                  {!rows.length && (
                    <tr>
                      <td colSpan={6}>
                        No assigned vendors match. Use Add Vendor to select from
                        master data.
                      </td>
                    </tr>
                  )}
        ...
        <div className="merchant-vendors">
          ...
          {modal?.type === "select" ? (
            <>
              ...
              {masterError && (
                <p role="alert" className="mv-error">
                  {masterError}
                </p>
              )}
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    const vendors = await getAvailableMerchantVendors(
                      merchantId,
                      { search: value },
                    );
                    ...
                    setMasterError(
                      e?.message || "Unable to search available vendors.",
                    );
                  } finally {
                    setMasterLoading(false);
                  }
                ...
                <table>
                  ...
                  <tbody>
                      ...
                      <tr key={vendor.id}>
                        <td>
                          <input
                            ...
                            disabled={busy || !active(vendor)}
                            checked={selection.includes(vendor.id)}
                            onChange={(e) =>
                              setSelection((old) =>
                                e.target.checked
                                  ? [...old, vendor.id]
                                  : old.filter((id) => id !== vendor.id),
                              )
                            }
                          />
                        </td>
                    ...
                    {!available.length && (
                      <tr>
                        <td colSpan={5}>
                          No unassigned vendors found in master data.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            modal?.vendor && (
              <dl className="mv-details">
                {Object.entries({
                  Vendor: modal.vendor.name,

                  Type: modal.vendor.type,

                  Contact: modal.vendor.contact,
            ...
            <div>
                  ...
                  onClick={() =>
                    save([
                      ...new Set([
                        ...
                        ...selection.filter((id) =>
                          vendors.some(
                            (vendor) => vendor.id === id && active(vendor),
                          ),
                        ),
                      ]),
                    ])
              ...
              {modal?.type === "remove" && (
                <button
                  ...
                  disabled={busy}
                  onClick={() =>
                    save(ids.filter((id) => id !== modal.vendor.id))
                  }
                >
